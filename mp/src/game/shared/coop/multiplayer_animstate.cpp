...
void CMultiPlayerAnimState::ComputeMainSequence()
{
    ...
    if ( (oldActivity == ACT_HL2MP_IDLE_CROUCH || oldActivity == ACT_HL2MP_IDLE ) && 
         (idealActivity == ACT_HL2MP_RUN || idealActivity == ACT_HL2MP_WALK_CROUCH ) )
    {
        ResetGroundSpeed();
    }
#endif
...
//    Anim_StateLog( "-------------%s: frame %d -----------------\n", bIsServer ? "Server" : "Client", gpGlobals->framecount );
    ...
    Anim_StatePrintf( iLine++, "Main: %s, %s, Cycle: %.2f\n", GetSequenceName( GetBasePlayer()->GetModelPtr(), GetBasePlayer()->GetSequence() ), GetSequenceActivityName( GetBasePlayer()->GetModelPtr(), GetBasePlayer()->GetSequence() ), GetBasePlayer()->GetCycle() );
    ...
    //Anim_StatePrintf( iLine++, "Sequence: %i, PlaybackRate: %.2f, AnimTime: %.2f", GetBasePlayer()->GetSequence(), GetBasePlayer()->GetPlaybackRate(), GetBasePlayer()->GetAnimTime() );
